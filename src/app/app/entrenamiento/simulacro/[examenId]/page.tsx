...
export default function SimulacroPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['simulacro-preguntas', examenId],
    queryFn: async () => {
      const res = await api.get(`/temas/examenes/${examenId}/preguntas`);
      return res.data;
    },
  });

  const ejercicioInfo = data?.convocatoria?.ejercicios?.find((e: any) => e.numero === data?.examen?.parte);
  const tiempoMinutos = ejercicioInfo?.tiempoMinutos ?? null;
  ...
  const corregir = useMutation({
    mutationFn: async () => {
      ...
      const res = await api.post(`/temas/examenes/${examenId}/corregir`, { respuestas: respuestasArray });
      return res.data;
    },
    onSuccess: (data) => {
      setResultado(data);
        ...
        <div style={{ maxWidth: 480, margin: '0 auto', padding: '1.5rem' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 13, color: TEXT_MUTED }}>
              {data.examen.anyo} · Ejercicio {data.examen.parte}
            </div>
          </div>
          ...
          <div style={{ background: '#F3E8FF', borderRadius: 14, padding: 14, marginBottom: 24, fontSize: 12, color: '#5B21B6', lineHeight: 1.6 }}>
            💡 Este es un examen real de convocatorias anteriores. Se corregirá exactamente igual que en el examen oficial, aplicando la penalización configurada. Una vez empieces, el tiempo correrá sin pausa.
          </div>
          ...
          <div style={{ maxWidth: 560, margin: '0 auto', padding: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              {indiceActual < preguntas.length - 1 ? (
                ...
                <button
                  onClick={() => {
                    if (confirm('¿Finalizar el examen y ver tu resultado?')) corregir.mutate();
                  }}
